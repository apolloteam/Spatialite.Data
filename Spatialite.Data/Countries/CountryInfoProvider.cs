namespace Spatialite.Data.Countries
{
    ...
    public class CountryInfoProvider
    {
        ...
        private const string QuerySql = @"SELECT c.* FROM CountryInfo c 
                                        INNER JOIN TimeZones t ON c.ISO = t.CountryCode 
                                ...
                                using (SQLiteCommand cmd = conn.CreateCommand())
                                {
                                    ...
                                    using (SQLiteDataReader dr = cmd.ExecuteReader())
                                    {
                                        while (dr.Read())
                                        {
                                            ...
                                            country.Area = valueAux == DBNull.Value
                                                               ? null
                                                               : (decimal?)Convert.ToDecimal(valueAux);
                                            valueAux = dr.GetValue((int)CountryInfoField.Population);
                                            country.Population = valueAux == DBNull.Value
                                                                     ? null
                                                                     : (decimal?)Convert.ToDecimal(valueAux);
        ...
        public CountryInfo GetCountry(decimal latitude, decimal longitude)
        {
            ...
            using (SQLiteConnection conn = new SQLiteConnection(this.connection))
            {
                ...
                conn.LoadExtension(DataConfiguration.LibSpatialite);
        ...
        /// <summary>The get lib spatialite.</summary>
        /// <returns>The <see cref="string" />.</returns>
        private string GetLibSpatialite()
        {
            // System.IO.File.Exists()
            // LibSpatialiteDll
            if (this.libPath == null)
            {
                this.libPath = LibSpatialiteDll;
                if (!File.Exists(this.libPath))
                {
                    string location = Assembly.GetExecutingAssembly().Location;
                    this.libPath = Path.Combine(location, LibSpatialiteDll);
                    if (!File.Exists(this.libPath))
                    {
                        this.libPath = Path.Combine(location, "bin", LibSpatialiteDll);
                        if (!File.Exists(this.libPath))
                        {
                            this.libPath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, LibSpatialiteDll);
                            if (!File.Exists(this.libPath))
                            {
                                this.libPath = Path.Combine(
